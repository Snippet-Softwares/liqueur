...

{
...

    {
        ...
        if ($request->ajax()) {
            ...
            $query = TransactionSellLine::join(
                ...
                //->leftjoin('tax_rates', 'tax_rates.id', '=', '1')
                ...
                ->select(
                    ...
                    //'v.sell_price_inc_tax as unit_sale_price',
                    ...
                    DB::raw('((transaction_sell_lines.line_discount_amount) * (transaction_sell_lines.quantity - transaction_sell_lines.quantity_returned)) as total_discount_amount'),
                    ...
                    DB::raw('MAX(v.dpp_inc_tax) as unit_purchase_price'),

                    DB::raw('(MAX(v.dpp_inc_tax) * (transaction_sell_lines.quantity - transaction_sell_lines.quantity_returned) ) as total_purchase_price'),

                    //'tax_rates.amount as item_tax',

                    DB::raw('(((transaction_sell_lines.quantity - transaction_sell_lines.quantity_returned) * transaction_sell_lines.unit_price_inc_tax) * (tax_rates.amount / 100) ) as item_tax'),

                    'u.short_name as unit',

                    DB::raw('(((transaction_sell_lines.quantity - transaction_sell_lines.quantity_returned) * transaction_sell_lines.unit_price_inc_tax) * ((tax_rates.amount + 100) / 100) ) as unit_sale_price'),

                    //original code
                    //DB::raw('((transaction_sell_lines.quantity - transaction_sell_lines.quantity_returned) * v.sell_price_inc_tax) as subtotal')
                   
                    //this is the brute force approach of what is expected
                    //DB::raw('(((transaction_sell_lines.quantity - transaction_sell_lines.quantity_returned) * transaction_sell_lines.unit_price_inc_tax) * 1.16) as subtotal')
                    
                    //testing ground
                    DB::raw('(((transaction_sell_lines.quantity - transaction_sell_lines.quantity_returned) * transaction_sell_lines.unit_price_inc_tax) * ((tax_rates.amount + 100) / 100) ) as subtotal'),

                    DB::raw('((((transaction_sell_lines.quantity - transaction_sell_lines.quantity_returned) * transaction_sell_lines.unit_price_inc_tax) * ((tax_rates.amount + 100) / 100)) - (MAX(v.dpp_inc_tax) * (transaction_sell_lines.quantity - transaction_sell_lines.quantity_returned) ) ) as profit_loss'),
            ...
            return Datatables::of($query)
                ...
                ->editColumn('current_stock', function ($row) {
                  ...
                  if ($row->enable_stock) {
                      ...
                      return ' ' . (float) $row->current_stock . ' ' .$row->unit;
                 ...
                 ->editColumn('subtotal', function ($row) {
                     ...
                     //return '<span class="display_currency row_subtotal" data-currency_symbol = true data-orig-value="' . ($row->subtotal - $row->discount_amount) . '">' . ($row->subtotal - $row->discount_amount) . '</span>';
                     return '<span class="display_currency row_subtotal" data-currency_symbol = true data-orig-value="' . ($row->subtotal) . '">' . ($row->subtotal) . '</span>';
                ...
                ->editColumn('total_purchase_price', function ($row) {
                    ...
                    return '<span class="display_currency row_total_purchase_price" data-currency_symbol = true data-orig-value="' . ($row->total_purchase_price) . '">' . ($row->total_purchase_price) . '</span>';

                })

                ->editColumn('profit_loss', function ($row) {

                    return '<span class="display_currency row_profit_loss" data-currency_symbol = true data-orig-value="' . ($row->profit_loss) . '">' . ($row->profit_loss) . '</span>';
                ...
                ->editColumn('total_discount_amount', function ($row) {

                        //return '<span class="display_currency" data-currency_symbol = true>' . $row->total_discount_amount . '</span>';
    
                        return '<span class="display_currency row_total_discount" data-currency_symbol = true data-orig-value="' . ($row->total_discount_amount) . '">' . ($row->total_discount_amount) . '</span>';

                })    

                ->editColumn('tax', function ($row) {

                    //return '<span class="display_currency row_tax" data-currency_symbol = true data-orig-value="' .(float)$row->item_tax . '">' . ($row->item_tax) .'" data-unit="'.$row->item_tax.'</span>';

                     return '<span class="display_currency" data-currency_symbol = true >'.

                            (float)$row->item_tax.

                       '</span>'.'<br>'.'<span class="tax" data-orig-value="'.(float)$row->item_tax.'" > </span>';
  
                })

                ->rawColumns(['invoice_no', 'unit_sale_price', 'subtotal', 'sell_qty', 'unit_purchase_price','total_purchase_price','profit_loss','discount_amount', 'total_discount_amount', 'unit_price', 'tax'])
     ...
     * Shows product purchase report with profit and loss
    ...
    public function getproductSellReportWithMarging(Request $request)
        ...
        if ($request->ajax()) {
            ...
            return Datatables::of($query)
                ...
                ->editColumn(
                    ...
                    '<div style="white-space: nowrap;"><span class="display_currency" data-currency_symbol="true">{{round($min_purchase_price)}}</span> @if($max_purchase_price != $min_purchase_price && $type == "variable") -  <span class="display_currency" data-currency_symbol="true">{{$max_purchase_price}}</span>@endif </div>'
                 ...
                 ->editColumn('total_purchase_price', function ($row) {
                    ...
                    return '<span class="display_currency row_total_purchase_price" data-currency_symbol = true data-orig-value="' . ($row->min_purchase_price * $row->sell_qty) . '">' . ($row->min_purchase_price * $row->sell_qty) . '</span>';
                ...
                ->editColumn('profit_loss', function ($row) {
                    ...
                    return '<span class="display_currency row_profit_loss" data-currency_symbol = true data-orig-value="' . (($row->subtotal - $row->discount_amount) - ($row->min_purchase_price * $row->sell_qty)) . '">' . (($row->subtotal - $row->discount_amount) - ($row->min_purchase_price * $row->sell_qty)) . '</span>';
                ...
                ->rawColumns(['invoice_no', 'profit_loss', 'unit_sale_price', 'total_purchase_price', 'purchase_price', 'subtotal', 'sell_qty', 'discount_amount', 'unit_price', 'tax'])
        ...
        if ($request->ajax()) {
            ...
            $query = TransactionSellLine::join(
                ...
                ->select(
                    ...
                    DB::raw('SUM(transaction_sell_lines.line_discount_amount) as discount_amount'),
            ...
            return Datatables::of($query)
                ...
                ->editColumn('total_discount', function ($row) {
                    ...
                    return '<span class="display_currency" data-currency_symbol = true data-orig-value="' . $row->discount_amount . '">' . $row->discount_amount . '</span>';
                ...
                ->rawColumns(['current_stock', 'total_discount', 'subtotal', 'total_qty_sold'])
